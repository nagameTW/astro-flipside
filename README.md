...
上班時，你回信、開會，把職稱印在名片上。下班後，你練琴、打球，或用一個只有同好才認得的 ID 上線。
...
就像一枚硬幣，正面是專業，反面是熱愛。兩面都是真實的自己，只需要翻個面。

因此，在關於頁面中，Flipside 設計了一顆切換按鈕。點擊後，頭像會隨之翻轉，Work 面緩緩收起，Life 面自然展開。履歷、作品與興趣共存於同一個網址，訪客不必接受單一版本的你，而是可以自由選擇，從不同角度認識你。

Flipside 是一個簡潔、易於客製化，並對中文使用者相當友善的 Astro 主題。它源自我的個人網站專案，我移除了所有私人內容，保留了開發過程中累積的設計思考與技術實作，將它整理成一個可以直接開始使用的網站起點，希望讓更多人能打造一個既能展現專業，也能保留個人特色的個人網站。
