...
上班時，你回信、開會，把職稱印在名片上；下班後，你練琴、打球，或用
一個只有同好才認得的 ID 上線。

這兩個都是你，卻很少同時出現在同一個地方。

多數個人網站只有一種版型、一種敘事方式，於是我們總得取捨：要展現
專業，還是分享熱愛？最終，只能讓其中一面代表自己。

Flipside 的核心理念很簡單：何必妥協。

就像一枚硬幣，正面是專業，反面是熱愛；兩面都是真實的自己，只需要
翻個面。

因此，在「關於（About）」頁面中，Flipside 設計了一顆切換按鈕。點擊
後，頭像會隨之翻轉，Work 面緩緩收起，Life 面自然展開。履歷、作品與
興趣共存於同一個網址，訪客不必接受單一版本的你，而是可以自由選擇，
從不同角度認識你。

Flipside 是一個簡潔、易於客製化，並對中文使用者相當友善的 Astro
主題。它源自我的個人網站專案，我移除了所有私人內容，保留了開發過程
中累積的設計思考與技術實作，將它整理成一個可以直接開始使用的網站
起點，希望讓更多人能打造一個既能展現專業，也能保留個人特色的個人
網站。
...
消失，回第 3 步檢查 `base`。內容都住在〈[專案結構](#專案結構)〉標了
箭頭的三個地方；`src/config.ts` 的其餘欄位（站名、描述、社群連結、
功能旗標）都有註解，照著填就行。範例文章讀完就可以刪。
...
- [ ] `/projects/` 作品集頁（`src/data/projects.ts` 與 `ProjectCard.astro`
      的骨架已就緒，等有實際內容再接上路由）
...
專案連結：<https://github.com/nagameTW/astro-flipside>
