...
<table>
  <tr>
    <td><img src="docs/screenshot-work.png" alt="關於頁的 Work 面：履歷、技能、經歷"></td>
    <td><img src="docs/screenshot-home-light.png" alt="關於頁的 Life 面：興趣、收藏、生活"></td>
  </tr>
  <tr>
    <td align="center"><sub>Work 面</sub></td>
    <td align="center"><sub>Life 面，同一個網址</sub></td>
  </tr>
</table>
...
<details>
<summary>更多截圖：暗色主題、文章頁面</summary>

<table>
  <tr>
    <td><img src="docs/screenshot-home-dark.png" width="380" alt="首頁，Life 面，暗色主題"></td>
    <td><img src="docs/screenshot-post.png" width="380" alt="文章頁面，含目錄與程式碼區塊"></td>
  </tr>
  <tr>
    <td align="center"><sub>暗色主題</sub></td>
...
2. 編輯 `src/config.ts`：網站網址、base 路徑、標題、導覽列、社群連結、
   功能旗標。詳見〈[設定參考](#設定參考)〉。
3. 填入你的內容。
   - `src/data/*`：兩個面的「關於」內容（`about.ts`、`life.ts`）、
     相簿（`gallery.ts` 搭配 `public/gallery/`）、獎盃牆、裝備清單。
   - `src/content/blog/`：寫你的文章，寫完後刪掉範例文章。
4. 到 repo 的 **Settings → Pages**，把 **Source** 設成 **GitHub
   Actions**。`npm run dev` 在本機預覽；推上 `main` 就會部署（詳見
   〈[部署](#部署)〉）。
...
- 資料驅動：在 `src/data/gallery.ts` 列出照片，檔案放 `public/gallery/`
- 點圖片會用燈箱放大顯示
