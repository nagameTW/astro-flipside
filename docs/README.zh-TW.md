...
- Work/Life 切換鈕，附 3D 大頭貼翻轉動畫。兩個面都用同一組 12 種通用內容
  ...
  markdown（自由格式），加上 Steam／Last.fm 專用區塊。要換內容，改
  資料檔就好，不用動元件。
...
- 選用、預設關閉的功能旗標：KaTeX 數學公式、Mermaid 圖表、giscus 留言、
  Steam／Last.fm 資料區塊。旗標關閉時，這些功能不會佔用任何 bundle
  大小。
...
**Steam／Last.fm 資料**（Life 面；Music/Steam 區塊在沒有資料前會自動
隱藏）：

- Repo **secrets**（Settings → Secrets and variables → Actions →
  Secrets）：`STEAM_API_KEY`、`LASTFM_API_KEY`。只要設定你會用到的那幾
  個，沒設定 secret 的來源會被悄悄跳過。
- Repo **variables**（同一頁的 Variables）：`STEAM_ACCOUNTS`（逗號分隔的
  SteamID）、`EXCLUDED_APPIDS`、`LASTFM_USER`。
