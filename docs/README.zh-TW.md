...
- Work/Life 切換鈕，附 3D 大頭貼翻轉動畫。兩個面都用同一組 12 種通用內容
  區塊組成：text（文字）、chips（標籤）、kv（鍵值）、timeline（時間軸）、
  highlights（亮點）、cards（卡片）、stats（數據）、links（連結）、
  markdown（自由格式），加上 Steam／osu!／Last.fm 專用區塊。要換內容，改
  資料檔就好，不用動元件。
...
- 選用、預設關閉的功能旗標：KaTeX 數學公式、Mermaid 圖表、giscus 留言、
  Steam／osu!／Last.fm 資料區塊。旗標關閉時，這些功能不會佔用任何 bundle
  大小。
...
**Steam／osu!／Last.fm 資料**（Life 面；Music/Steam/osu! 區塊在沒有資料前
會自動隱藏）：

- Repo **secrets**（Settings → Secrets and variables → Actions →
  Secrets）：`STEAM_API_KEY`、`OSU_CLIENT_ID`、`OSU_CLIENT_SECRET`、
  `LASTFM_API_KEY`。只要設定你會用到的那幾個，沒設定 secret 的來源會被
  悄悄跳過。
- Repo **variables**（同一頁的 Variables）：`STEAM_ACCOUNTS`（逗號分隔的
  SteamID）、`EXCLUDED_APPIDS`、`OSU_USER_ID`、`OSU_MODE`（模式：
  `osu`/`taiko`/`fruits`/`mania`，預設 `osu`）、`LASTFM_USER`。
- `.github/workflows/gamestats.yml` 每天（也可手動觸發）執行
  `scripts/fetch_gamestats.py`，寫入 `src/data/gamestats.json`，有變動就
  提交，並觸發重新部署。
...
就是你在本地能跑的那四個檢查：`npm run check && npm run build && npm test`
加上 `python3 scripts/test_fetch_gamestats.py`。
